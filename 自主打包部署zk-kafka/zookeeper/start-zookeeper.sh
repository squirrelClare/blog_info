...
PURGE_INTERVAL=0
SERVERS=1
AUTH_PROVIDER="org.apache.zookeeper.server.auth.SASLAuthenticationProvider"
REQ_CLIENT_AUTH="sasl"
JAAS_LOGIN_RENEW=3600000
SERVER_JVMFLAGS="-Djava.security.auth.login.config=/opt/zookeeper/conf/jaas.conf"
CLIENT_JVMFLAGS="-Djava.security.auth.login.config=/opt/zookeeper/conf/jaas.conf"
...
function create_config() {
    ...
    echo "authProvider.1=$AUTH_PROVIDER"  >> $CONFIG_FILE
    echo "requireClientAuthScheme=$REQ_CLIENT_AUTH"  >> $CONFIG_FILE
    echo "jaasLoginRenew=$JAAS_LOGIN_RENEW" >> $CONFIG_FILE
...
function create_jvm_props() {
    ...
    echo "SERVER_JVMFLAGS=$SERVER_JVMFLAGS" >> $JAVA_ENV_FILE
    echo "CLIENT_JVMFLAGS=${CLIENT_JVMFLAGS}" >> $JAVA_ENV_FILE
}
